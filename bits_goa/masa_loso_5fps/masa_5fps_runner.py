...
================================================================
Reads 30-fps NPY files but applies fps_stride=6 at the dataset level
→ model effectively sees 5-fps sequences.
This matches the 5-fps popsign baseline for apples-to-apples comparison.
"""
...
config = {
    # Data — 5 FPS
    "max_frames"     : 30,            # was 150 at 30fps; 30 frames at 5fps = 6 sec
    "fps_stride"     : 6,             # 30fps → 5fps
    "num_workers"    : 6,

    # Training — same proven config as 30fps LOSO
    "batch_size"     : 64,
    "epochs"         : 60,
    "warmup_epochs"  : 3,
    "min_lr_ratio"   : 0.05,
...
print("=" * 60)
print(f"  MASA LOSO 5-FPS Training  |  {ts}")
print(f"  DATA_ROOT   : {DATA_ROOT}")
print(f"  RESULTS_DIR : {RESULTS_DIR}")
print(f"  fps_stride  : {config['fps_stride']}  (30fps → 5fps)")
print(f"  max_frames  : {config['max_frames']}  (= {config['max_frames']/5:.1f} sec at 5fps)")
...
for fold_idx, assignment in enumerate(fold_assignments):
    ...
    avg = (time.time() - overall_start) / completed_folds
    remaining = (num_folds - fold_idx - 1) * avg
    print(f"\n  ⏱ Fold {fold_idx} time: {fold_time/60:.1f} min   "
          f"|  Avg: {avg/60:.1f} min  "
          f"|  ETA: {remaining/60:.1f} min ({remaining/3600:.1f} h)")
...
print("\n\n" + "=" * 70)
print("  FINAL MASA 5-FPS LOSO K-FOLD SUMMARY")
print("=" * 70)

print(
    f"\n  {'Fold':<6} {'Test':<15} {'TrTop1':>7} {'TrTop5':>7} "

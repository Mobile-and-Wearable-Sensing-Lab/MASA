"""
masa_train.py — Train one LOSO fold (MASA experiment, 5fps-aware)
=====================================================
Defensive version with:
  ...
  • Respects config["fps_stride"] (default 1) for on-the-fly FPS reduction
"""
...
def train_one_fold(
    ...
    # Read fps_stride from config (default 1 = no FPS change)                # ← NEW
    fps_stride = int(config.get("fps_stride", 1))                             # ← NEW
    ...
    print(f"  FOLD {fold_idx}"
          ...
          f"  |  Epochs: {epochs}  |  fps_stride: {fps_stride}")        # ← NEW
    print(f"{'='*60}")
    ...
    if bad_batches > 0:
        print(f"  Total bad batches skipped: {bad_batches}")
